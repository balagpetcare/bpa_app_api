...
function mapEpsStatus(status: string | undefined): "SUCCESS" | "FAILED" | "CANCELLED" {
...
export async function createIntent(req: PaymentIntentRequest): Promise<PaymentIntentResponse> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "EPS payment initialization failed";
...
export async function checkTransactionStatus(input: {
  ...
  if (!isEpsConfigured()) return null;
...
export function parseCallbackQuery(query: Record<string, string>): VerifiedPaymentEvent | null {
